...
TMP_DB_PATH = '/tmp/students.db' if os.name != 'nt' else LOCAL_DB_PATH
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path.endswith('/api/search') or path.endswith('/api/search/'):
            self.handle_search_api(query_params)
        elif path.endswith('/api/stats') or path.endswith('/api/stats/'):
